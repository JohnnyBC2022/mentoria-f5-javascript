...
## Quinceavo paso: añadir un spinner.

Vamos a añadir un spinner para simular un envío, para ello vamos a usar la página web <a href="https://tobiasahlin.com/spinkit/" target="_blank">https://tobiasahlin.com/spinkit/</a>.

Dentro del enlace viene un enlace con el código HTML y CSS y solo tenemos que copiar el que más nos guste.

En mi caso utilizo este HTML:
...
Observa que englobo el spinner dentro de una div que contiene unas clases de **_TailWind_** y entre ellas está la clase **_"hidden"_** que hace que por defecto no se muestre el spinner.
...
La única modificación que hago es en la línea 22, donde le cambio la propiedad css **_background-color_**.
...
Como vamos a manejar ese spinner, vamos declarar una variable que nos permita seleccionarlo.

```javascript
const spinner = document.querySelector("#spinner");
```

Dentro de la función, le añadimos le añadimos la clase "flex" y le quitamos la clase "hidden".

Ahora, vamos a simular que pasa un tiempo, se envía la información y desaparece el spinner.
...
Ahora, vamos a reiniciar el formulario, y podemos copiar el mismo código que usamos antes.
